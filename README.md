...
- `npm run check:backend:readiness`: kiểm tra live health endpoint (`/health`) khi chạy backend mode; tự skip nếu `USE_MOCK_API=true`.
- `npm run check:staging:smoke`: smoke test tích hợp staging (`health + properties`), mặc định skip nếu chưa bật `STAGING_SMOKE_ENABLED=true`.
- `npm run generate:funnel-dashboard`: tạo dashboard funnel Search -> Detail -> Save -> Contact tại `docs/funnel-dashboard.md`.
- `npm run check:kpi-alerts`: kiểm tra ngưỡng KPI funnel và báo động nếu thấp hơn ngưỡng cấu hình.
...
- `npm run app:smoke`: chạy smoke test lớp App shell/runtime/controller để xác nhận luồng ứng dụng hoạt động.
- `npm run debug:deep`: chạy vòng lặp debug sâu 5 vòng (start + app smoke + verify + readiness + progress gate).
- `npm run ops:health`: chạy health-check nhanh cho sandbox (git status, runtime versions, process, ports, core scripts availability).
...
- `ANALYTICS_ENDPOINT`: endpoint nhận analytics event (để trống nếu chỉ track local in-memory).
- `ANALYTICS_WRITE_KEY`: write key/token cho analytics transport.
- `ANALYTICS_TIMEOUT_MS`: timeout gửi analytics event.
- `TOKEN_ENCRYPTION_KEY`: secret dùng để derive AES key cho token encryption ở storage layer.
- `CONTACT_RATE_LIMIT_WINDOW_MS`: cửa sổ thời gian rate-limit cho contact/book-visit (ms).
- `CONTACT_RATE_LIMIT_MAX_REQUESTS`: số request tối đa trong mỗi cửa sổ rate-limit.
- `SENTRY_DSN`: endpoint DSN cho crash/error monitoring.
- `SENTRY_RELEASE`: version release gửi kèm sự kiện monitoring.
- `STAGING_SMOKE_ENABLED`: bật/tắt smoke test staging.
- `STAGING_API_BASE_URL`: base URL staging dùng cho smoke test.
- `STAGING_SMOKE_TIMEOUT_MS`: timeout cho mỗi request trong smoke test staging.
...
- Contract API chuẩn hóa cho luồng MVP nằm tại `docs/openapi.yaml`.
- Gateway HTTP đã hỗ trợ:
  - `X-Request-Id` per request để trace log theo request context.
  - Retry policy theo `criticality` của endpoint.
  - `Idempotency-Key` cho action ghi (`save/remove/contact/book-visit`) để giảm double-submit.
...
- Hồ sơ pháp lý/phát hành bổ sung:
  - `docs/privacy-policy.md`
  - `docs/terms-of-service.md`
  - `docs/rollback-plan.md`
  - `docs/sourcemap-strategy.md`
  - `docs/owasp-asvs-mvp-checklist.md`
...
- Dùng vòng lặp debug liên tục theo tiến triển (có log + report từng vòng):
  - `bash skills/human-system-debug/scripts/debug_iteration_loop.sh --loops 3`
  - hoặc preset sâu hơn: `npm run debug:deep`
